...
final class AuthFilterMiddleware extends BaseMiddleware
{
    ...
    private function Filter(array $data) {
        ...
        if (!isset($data["userName"])) 
            return array_push($errors ,$this->json("Where is userName???" , []));
        if (!isset($data["email"])) 
        ...
        if (!$this->MinMaxStr($data["password"],8,25)) {
            return $this->json("password have to (lengt min 8 max 25)" , []);
        }
        if (!preg_match("/[0-9]/",$data["password"])) {
            return $this->json("password have to minimum One number" , []);
        }
        if (!preg_match("/[0-9]/",$data["password"])) {
            return $this->json("password have to minimum One number" , []);
        }
        if (!preg_match("/[A-Z]/",$data["password"])) {
            return $this->json("password have to minimum One upper case" , []);
        }
        if (!preg_match("/[a-z]/",$data["password"])) {
            return $this->json("password have to minimum One lower case" , []);
        }
    }
