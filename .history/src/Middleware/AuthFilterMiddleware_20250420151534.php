...
final class AuthFilterMiddleware extends BaseMiddleware
{
    ...
    private function Filter(array $data) {
        if (!isset($data["userName"])) {
            return $this->json("Where is userName???" , ["userName"=>$data["userName"]]);
        }
        if (!isset($data["email"])) {
            return $this->json("Where is email???" , ["userName"=>$data["userName"]]);
        }
        if (!isset($data["password"])) {
            return $this->json("Where is password???" , ["userName"=>$data["userName"]]);
        }
        if (!isset($data["confirmedPassword"])) {
            return $this->json("Where is confirmedPassword???" , ["userName"=>$data["userName"]]);
        }
