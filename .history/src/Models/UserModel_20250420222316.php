...
final class UserModel{
    ...
    public static function CreateManuelUser(array $data,string $CodePin)  {
        ...
        $sth=$conn->prepare("INSERT INTO user(userName,email,password,emailVerificationCode,createdDate)VALUES(?,?,?,?,'NOW()')");
    ...
    public static function GetLastUserAdded():UserEntities{
        $conn = DBConnection::GetConnection("sinox");
        return new UserEntities($conn->query("SELECT * FROM user WHERE userId == MAX(userId);")->fetchAll(PDO::FETCH_ASSOC));
    }
}
?>