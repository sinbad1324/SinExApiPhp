...
final class UserModel{
    ...
    public static function CreateManuelUser(array $data,string $CodePin)  {
        var_dump(static::FindWithName($data["userName"]));
    ...
    public static function FindWithId($id) : UserEntities | null {
        ...
        $sth->execute();
        $data = $sth->fetch(PDO::FETCH_ASSOC);        if ($data && count($data)>1){
    ...
    public static function GetLastUserAdded():array{
        $conn = DBConnection::GetConnection("sinox");
        return $conn->query("SELECT * FROM user WHERE userId = (SELECT MAX(userId) FROM user);")->fetchAll(PDO::FETCH_ASSOC);
    }
}
?>