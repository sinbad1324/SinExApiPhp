...
final class ManuelAuthServices
{
    ...
    public function VerifieCode($code, $id) {
    ...
    public function RemakeVerificationCode($code, $id) {
        $ud = UserModel::FindWithId($id);
        if ($ud) {
            $user = new UserEntities($ud);
            if ($user->MailPinCodeIsSame($code)) {
                    return static::json("Nous avons pas réussit a vous verifier voulez vous essayer plus tard!", ["code"=>user->RemakeNewCode()]); // ne pas donne le code en deploiment 
            }
            else
                return static::json("Ce code est invalide!", []); 
        }
