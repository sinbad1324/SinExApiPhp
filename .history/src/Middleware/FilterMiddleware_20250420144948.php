...
final class FilterMiddleware extends BaseMiddleware
{
    protected function  Execute(Request $request, RequestHandler $handler): Response
    {
            ...
            // withAttribute return une nouvelle instance d request
            $request=$request
            ->withAttribute('dataQuery' , "edwedwedw")
            ->withAttribute('dataBody' , "ewfewffew");
    ...
    private function Filter(array $data) {
        foreach ($data as $key => $value) {
            if (is_string($value)) filter_var($value , FILTER_SANITIZE_SPECIAL_CHARS);
            if (is_string($key)) filter_var($key , FILTER_SANITIZE_SPECIAL_CHARS);
        }
    }
}
